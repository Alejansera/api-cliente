...
export class ProductoMainComponent implements OnInit {
  ...
  constructor(private servicio:ProductoService, public servicioUsuario:UsuarioService, private router:Router) { }

  ngOnInit(): void {

    this.servicio.mostrarProducto().subscribe(
      resp => { this.productos=resp ;console.log("Esto me responde al mostrar Productos"+this.productos)}
    );
